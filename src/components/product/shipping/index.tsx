      ...
      <div className="product-shipping-options">
        {
          shipping.map((option) => {
            ...
            return <ShippingOption key={Math.random()} name={option.name} price={option.price} etaMax={option.etaMax} selected={isSelected} onClick={() => { setSelectedOption(option) }} etaMin={option.etaMin} />
          })
        }
      </div>
    </div>
