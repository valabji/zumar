...
export default function breadcrumb ({ id }: breadcrumb): ReactElement {
  ...
  return <div className="breadcrumb-container" >
        ...
        {categoryList.map((category) => {
          return <span key={Math.random()}><span className="breadcrumb-item" style={{ cursor: 'default' }}>{'>'}</span><a href={'/category/' + category.id.toString()} className="breadcrumb-item" >{category.name}</a></span>
        })}
    </div>
}
